...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await pool.query(`
      SELECT DISTINCT ON (place_name, place_type)
        place_name,
        place_type,
        latitude,
        longitude,
      ...
      ORDER BY place_name, place_type, recorded_at DESC
      LIMIT 200
    `);
    ...
    console.error('GET /api/spaces/[id] failed:', error);

    return NextResponse.json(
      {
        success: false,
        message: 'Failed to fetch space detail from database.',
      },
      { status: 500 }
    );
  }
